...
Проект покрыт PHPUnit-тестами для ключевой бизнес-логики, HTTP-слоя и инфраструктурных компонентов.

Тесты проверяют:

* `ContactController` — обработку HTTP-запросов и ответы API;
* `ExceptionListener` — обработку HTTP- и серверных ошибок, debug/prod поведение;
* `HuggingFaceModerationService` — взаимодействие с сервисом модерации;
* `MessageDto` — валидацию входных данных;
* `MessageService` — валидацию, сохранение сообщений и отправку уведомлений;
* `NotificationHandler` — обработку асинхронных уведомлений;
* `RateLimiterService` — ограничение частоты запросов;
* `RequestLoggerListener` — логирование входящих запросов и исходящих ответов.
